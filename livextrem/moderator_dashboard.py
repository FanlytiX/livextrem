...
def twitch_login():
    ...
    # 1. ÄNDERUNG: Check auf Argumente vom Launcher
    # Wir erwarten jetzt mind. 8 Argumente (Scriptname + 7 Daten)
    if len(sys.argv) >= 8:
        try:
            ...
            # Falls das schiefgeht, normaler Login unten

    # 2. Fallback: Normaler Login via Browser (wenn Dashboard allein gestartet wird)
    try:
...
def init_database():
    ...
    try:
        ...
        # Twitch Login (Checkt nun Launcher-Argumente zuerst)
        if not twitch_login():
            return False
...
mod_info_label = ctk.CTkLabel(mod_info_frame, text="👤 Moderator", font=("Arial", 12, "bold"))
mod_info_label.pack(pady=(8, 2))

mod_name_label = ctk.CTkLabel(mod_info_frame, text="...", font=("Arial", 14))
...
btn_actions = HoverButton(sidebar, text="⚡  Moderationsaktionen", width=180, corner_radius=6)
...
def show_chat_monitor():
    ...
    if error:
        ...
        ctk.CTkLabel(error_container, text="⚠️ Chat-Fehler", 
                    font=("Arial", 20, "bold"), text_color="#ff6b6b").pack(pady=(30, 10))
        ctk.CTkLabel(error_container, text=error, 
                    font=("Arial", 14), wraplength=600).pack(pady=(0, 30))
        return
    ...
    if messages and "data" in messages and messages["data"]:
        for msg in reversed(messages["data"]):
            ...
            ctk.CTkLabel(header_frame, text=time_str, 
                        font=("Arial", 9), text_color=("gray40", "gray"), anchor="e").pack(side="right")
...
def show_actions():
    ...
    ctk.CTkLabel(timeout_form, text="Username:", font=("Arial", 14, "bold")).pack(pady=(10, 5))
    timeout_user_entry = ctk.CTkEntry(timeout_form, width=400, placeholder_text="z.B. troll123")
    timeout_user_entry.pack()
    
    ctk.CTkLabel(timeout_form, text="Dauer (Minuten):", font=("Arial", 14, "bold")).pack(pady=(15, 5))
    timeout_duration_entry = ctk.CTkEntry(timeout_form, width=400, placeholder_text="z.B. 10")
    timeout_duration_entry.pack()
    timeout_duration_entry.insert(0, "10")
    
    ctk.CTkLabel(timeout_form, text="Grund (optional):", font=("Arial", 14, "bold")).pack(pady=(15, 5))
    timeout_reason_entry = ctk.CTkEntry(timeout_form, width=400, placeholder_text="z.B. Spam im Chat")
    timeout_reason_entry.pack()
    
    timeout_status = ctk.CTkLabel(timeout_form, text="", font=("Arial", 12))
    ...
    ctk.CTkButton(timeout_form, text="⏸️ Timeout ausführen", 
                 font=("Arial", 14, "bold"), height=40, width=250,
                 fg_color="#ffa500", hover_color="#ff8c00",
                 command=execute_timeout).pack(pady=(20, 0))
    ...
    ctk.CTkLabel(ban_form, text="Username:", font=("Arial", 14, "bold")).pack(pady=(10, 5))
    ban_user_entry = ctk.CTkEntry(ban_form, width=400, placeholder_text="z.B. troll123")
    ban_user_entry.pack()
    
    ctk.CTkLabel(ban_form, text="Grund (optional):", font=("Arial", 14, "bold")).pack(pady=(15, 5))
    ban_reason_entry = ctk.CTkEntry(ban_form, width=400, placeholder_text="z.B. Wiederholte Regelverstöße")
    ban_reason_entry.pack()
    
    ban_status = ctk.CTkLabel(ban_form, text="", font=("Arial", 12))
    ...
    ctk.CTkButton(ban_form, text="🚫 Ban ausführen", 
                 ...
                 command=execute_ban).pack(pady=(20, 0))
    ...
    ctk.CTkLabel(unban_form, text="Username:", font=("Arial", 14, "bold")).pack(pady=(10, 5))
    unban_user_entry = ctk.CTkEntry(unban_form, width=400, placeholder_text="z.B. troll123")
    unban_user_entry.pack()
    
    unban_status = ctk.CTkLabel(unban_form, text="", font=("Arial", 12))
    ...
    ctk.CTkButton(unban_form, text="✅ Unban ausführen", 
                 ...
                 command=execute_unban).pack(pady=(20, 0))
